...
export default async function BlueSpymasterPage({
  params,
  searchParams,
}: SpymasterPageProps) {
  ...
  if (!(await isTeamTokenValid(sessionId, "blue", token))) {
    return <Unauthorized team="blue" />;
  }
  ...
  return (
    <SpymasterView
      ...
      team="blue"
      token={token}
      initialState={toSpymasterState(game, "blue")}
    />
  );
}
